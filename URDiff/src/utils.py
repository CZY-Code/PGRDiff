...
@torch.no_grad()
def TensorSVT_DCT(x, tau):
    B, C, H, W = x.shape
    x_dct = dct(x.permute(0, 2, 3, 1)).permute(0, 3, 1, 2) #[BCHW]
    w = torch.zeros_like(x_dct)
    for b in range(B):
        for c in range(C):
            spatial_slice = x_dct[b, c, :, :]
            U, S, Vh = torch.linalg.svd(spatial_slice)
            S_thresh = torch.max(S - tau, torch.zeros_like(S))
            bar_w = U @ torch.diag(S_thresh) @ Vh
            w[b, c, :, :] = bar_w
    result = idct(w.permute(0, 2, 3, 1)).permute(0, 3, 1, 2) #[BCHW]
    return result
