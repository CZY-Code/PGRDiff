...
class ResidualDiffusion(nn.Module):
    ...
    def ddim_sample(self, data, shape, last=True):
        ...
        img = x_input + math.sqrt(self.sum_scale) * torch.randn(shape, device=device) #x_t
        ...
        for time, time_next in tqdm(time_pairs, desc='sampling loop time step'):
            ...
            type = "use_low_rank_sampling" #use_pred_noise
            if type == "use_pred_noise":
                img = img - alpha * pred_res - (betas_cumsum - (betas2_cumsum_next - sigma2).sqrt()) * pred_noise + sigma2.sqrt() * noise
            elif type == "use_low_rank_sampling":
                LR_loss = torch.norm(TensorSVT_DCT(x_input - pred_res, tau=0.1) - x_start, p='fro')
                grad_xt = torch.autograd.grad(outputs=LR_loss, inputs=img, create_graph=False)[0]
                img = img - 200 * alpha * grad_xt
                img = img - alpha * pred_res - (betas_cumsum - (betas2_cumsum_next - sigma2).sqrt()) * pred_noise + sigma2.sqrt() * noise
